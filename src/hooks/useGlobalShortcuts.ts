...
export const useGlobalShortcuts = () => {
	...
	const customShortcutCallbacksRef = useRef<Map<string, () => void>>(
		new Map(),
	);

	const checkShortcutsRegistered = useCallback(async (): Promise<boolean> => {
		try {
			const registered = await invoke<boolean>(
			...
			try {
				...
				if (globalEventListeners.focus) {
					try {
						...
						console.warn(
							"Error cleaning up focus listener:",
							error,
						);
					}
				}
				if (globalEventListeners.screenshot) {
					try {
						...
						console.warn(
							"Error cleaning up screenshot listener:",
							error,
						);
					}
				}
				if (globalEventListeners.audio) {
					try {
						...
						console.warn(
							"Error cleaning up audio listener:",
							error,
						);
					}
				}
				if (globalEventListeners.systemAudio) {
					try {
						...
						console.warn(
							"Error cleaning up system audio listener:",
							error,
						);
					}
				}
				if (globalEventListeners.customShortcut) {
					try {
						...
						console.warn(
							"Error cleaning up custom shortcut listener:",
							error,
						);
					}
				}
						...
						if (globalScreenshotCallback) {
							try {
								Promise.resolve(globalScreenshotCallback())
									.catch((error) => {
										console.error(
											"Screenshot shortcut callback failed:",
											error,
										);
									})
									.then(() => {
				...
				const unlistenAudio = await listen(
				...
				const unlistenSystemAudio = await listen(
					"toggle-system-audio",
					() => {
				...
				const unlistenCustomShortcut = await listen<{ action: string }>(
					...
					(event) => {
						...
						const callback =
							globalCustomShortcutCallbacks.get(actionId);
						if (callback) {
							callback();
						} else {
							console.warn(
