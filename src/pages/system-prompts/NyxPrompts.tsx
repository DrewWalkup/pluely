...
const SELECTED_RUNNINGBORD_MODEL_STORAGE_KEY = "selected_nyx_model";
const SELECTED_RUNNINGBORD_PROMPT_STORAGE_KEY = "selected_nyx_prompt";

export const NyxPrompts = () => {
	const { setSystemPrompt, hasActiveLicense, setSupportsImages } = useApp();
	const [prompts, setPrompts] = useState<NyxPrompt[]>([]);
	const [isLoading, setIsLoading] = useState(false);
	const [error, setError] = useState<string | null>(null);
	const [lastUpdated, setLastUpdated] = useState<string | null>(null);
	const [selectedNyxPrompt, setSelectedNyxPrompt] =
		useState<NyxPrompt | null>(() => {
	...
	// Watch for changes in user's selected prompt and clear Nyx selection if needed
	useEffect(() => {
		const checkUserPromptSelection = () => {
			const userSelectedPromptId = safeLocalStorage.getItem(
				STORAGE_KEYS.SELECTED_SYSTEM_PROMPT_ID,
			);
			// If user has selected one of their own prompts, clear Nyx prompt selection
			if (userSelectedPromptId) {
				setSelectedNyxPrompt(null);
			}
		};
	...
	const fetchNyxPrompts = async () => {
		setIsLoading(true);
		setError(null);
		try {
			const response = await invoke<NyxPromptsResponse>("fetch_prompts");
			setPrompts(response.prompts);
			if (response.last_updated) {
				setLastUpdated(response.last_updated);
			}
		} catch (err) {
			console.error("Failed to fetch Nyx prompts:", err);
			setError(
				typeof err === "string" ? err : "Failed to fetch Nyx prompts",
			);
		} finally {
			setIsLoading(false);
		}
	...
	const handleSelectNyxPrompt = async (prompt: NyxPrompt) => {
		try {
			// Set the system prompt
			setSystemPrompt(prompt.prompt);
			setSelectedNyxPrompt(prompt);
			...
			// Save the selected Nyx prompt to local storage for persistence
			...
			const matchingModel = models.find(
				(model) =>
					model.model === prompt.modelId ||
					model.id === prompt.modelId,
			);
			...
			console.error("Failed to select Nyx prompt:", error);
		}
	};

	const handleCardClick = (prompt: NyxPrompt) => {
		handleSelectNyxPrompt(prompt);
	};

	const isPromptSelected = (prompt: NyxPrompt) => {
		return (
			selectedNyxPrompt?.title === prompt.title &&
			selectedNyxPrompt?.modelId === prompt.modelId
		);
	};
		...
		return (
			<div className="space-y-4 mt-6">
				...
				<Empty
					...
					description="Fetching Nyx default prompts"
				/>
			</div>
		);
	}
